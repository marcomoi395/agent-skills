...
## Auto-Update from tasks/ Directory

**Special workflow:** When invoked with a GitHub issue URL (e.g., `https://github.com/owner/repo/issues/10`), the skill automatically updates that issue based on the project's `tasks/` directory.

### Workflow

1. **Parse the issue URL** to extract `owner`, `repo`, and `issue_number`
2. **Scan `tasks/` directory** recursively to identify all task files
3. **Analyze task content** to understand:
...
5. **Update the issue** using:
   ...
   gh api repos/{owner}/{repo}/issues/{issue_number} \
     -X PATCH \
     -f body="Generated summary" \
     --jq '{number, html_url}'
   ```

### Format Requirements

When auto-updating from `tasks/`:

- ✅ **DO include:**
  - Brief 2-3 sentence overview of the project
  - High-level list of main areas (e.g., "Authentication, Dashboard, API Integration")
  - Current status if evident (e.g., "In Progress", "Planning")
...
Building a NestJS application with authentication, user management, and REST API endpoints. The project includes JWT-based auth, role-based access control, and a dashboard interface.
